...
<div class="p-4 bg-zinc-700 flex items-baseline">
    <h1 class="font-bold text-xl text-white mb-1">State employment explorer</h1> 
    <p class="ml-5 text-white">From the <a class="underline" href="https://observablehq.com/@themarshallproject/census-labor-data-release">Annual Survey of Public Employment & Payroll</a>, 2000-2022. <a class="underline" href="#about">About this site.</a></p>
</div>

<div class="p-4 flex items-center bg-zinc-200">
...
{:then stateData}
    ...
    <div class="flex border-b border-blue-900 bg-zinc-600 text-zinc-50" let:index let:style {style}>
        ...
        <div class="flex-initial ml-5 w-80 py-2.5">Full time employment</div>
        <div class="flex-initial ml-5 w-80 py-2.5">Full time pay / full time employees</div>
        <div class="flex-initial ml-5 w-80 py-2.5">Full time pay (monthly total)</div>
    </div>
    <VirtualList width="100%" height={itemSize * itemCount} itemCount={govFunctions.length} {itemSize}>
        <div slot="item" class="flex items-center border-b border-zinc-500" let:index let:style {style}>
            <div class="flex-none w-48 mr-5 uppercase text-sm"><div class="px-4 py-2">{govFunctions[index]}</div></div>
...
<div id="about" class="p-4 text-lg mx-auto max-w-2xl [&>p>a]:underline">
